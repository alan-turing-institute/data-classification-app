...
class TestUser:
    ...
    def test_project_role_for_participant(self, researcher):
        assert (
            researcher.user.project_permissions(researcher.project).role is ProjectRole.RESEARCHER
        )
        assert (
            researcher.user.project_participation_role(researcher.project) is ProjectRole.RESEARCHER
        )
