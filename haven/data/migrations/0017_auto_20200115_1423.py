# Generated by Django 2.1 on 2020-01-15 14:23
...
OPEN_GENERATE_NEW_GUIDANCE_TEXT = (
    '<p>Generating personal data means creating any <strong>new</strong> personal data, regardless of what data '
...
def migrate_questions(apps, schema_editor):
    insert_blank_guidance_if_necessary(apps, CLOSED_PERSONAL)
    insert_blank_guidance_if_necessary(apps, FINANCIAL_LOW)
    insert_blank_guidance_if_necessary(apps, FINANCIAL_LOW_PERSONAL)
    insert_blank_guidance_if_necessary(apps, INCLUDE_COMMERCIAL)
    insert_blank_guidance_if_necessary(apps, INCLUDE_COMMERCIAL_PERSONAL)
    insert_blank_guidance_if_necessary(apps, NO_REIDENTIFY)
    insert_blank_guidance_if_necessary(apps, NO_REIDENTIFY_ABSOLUTE)
    insert_blank_guidance_if_necessary(apps, NO_REIDENTIFY_STRONG)
    insert_blank_guidance_if_necessary(apps, OPEN_GENERATE_NEW)
    insert_blank_guidance_if_necessary(apps, OPEN_PUBLICATION)
    insert_blank_guidance_if_necessary(apps, PUBLIC_AND_OPEN)
    insert_blank_guidance_if_necessary(apps, PUBLISHABLE)
    insert_blank_guidance_if_necessary(apps, SOPHISTICATED_ATTACK)
    insert_blank_guidance_if_necessary(apps, SUBSTANTIAL_THREAT)

    # Update any guidance that need to change (including any new guidance)
    guidance = {g['name']: g for g in initial_guidance()}
    migrate_guidance(apps, CLOSED_PERSONAL, guidance[CLOSED_PERSONAL])
    migrate_guidance(apps, FINANCIAL_LOW, guidance[FINANCIAL_LOW])
    migrate_guidance(apps, FINANCIAL_LOW_PERSONAL, guidance[FINANCIAL_LOW_PERSONAL])
    migrate_guidance(apps, INCLUDE_COMMERCIAL, guidance[INCLUDE_COMMERCIAL])
    migrate_guidance(apps, INCLUDE_COMMERCIAL_PERSONAL, guidance[INCLUDE_COMMERCIAL_PERSONAL])
    migrate_guidance(apps, NO_REIDENTIFY, guidance[NO_REIDENTIFY])
    migrate_guidance(apps, NO_REIDENTIFY_ABSOLUTE, guidance[NO_REIDENTIFY_ABSOLUTE])
    migrate_guidance(apps, NO_REIDENTIFY_STRONG, guidance[NO_REIDENTIFY_STRONG])
    migrate_guidance(apps, OPEN_GENERATE_NEW, guidance[OPEN_GENERATE_NEW])
    migrate_guidance(apps, OPEN_PUBLICATION, guidance[OPEN_PUBLICATION])
    migrate_guidance(apps, PUBLIC_AND_OPEN, guidance[PUBLIC_AND_OPEN])
    migrate_guidance(apps, PUBLISHABLE, guidance[PUBLISHABLE])
    migrate_guidance(apps, SOPHISTICATED_ATTACK, guidance[SOPHISTICATED_ATTACK])
    migrate_guidance(apps, SUBSTANTIAL_THREAT, guidance[SUBSTANTIAL_THREAT])
